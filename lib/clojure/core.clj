...
(defn ^:private check-valid-options
  ...
  (when (seq (apply disj (apply hash-set (keys options)) valid-keys))
    (throw
      (ArgumentError.
        (str (apply str "Only these options are valid: "
                    (first valid-keys)
...
(defmacro defmulti
  ...
  (let [docstring   (if (string? (first options))
    ...
    (when (= (count options) 1)
      (throw (ArgumentError. "The syntax for defmulti has changed. Example: (defmulti name dispatch-fn :default dispatch-value)")))
    (let [options   (apply hash-map options)
          default   (get options :default :default)]
      (check-valid-options options :default)
      `(let [v# (def ~mm-name)]
         (when-not (and (.root v#) (instance? Hammock.MultiMethod (deref v#)))
           (def ~(with-meta mm-name m)
             (.new Hammock.MultiMethod ~(name mm-name) ~dispatch-fn ~default)))))))

(defmacro defmethod
  "Creates and installs a new method of multimethod associated with dispatch-value. "
  {:added "1.0"}
  [multifn dispatch-val & fn-tail]
  `(.add_method ~multifn ~dispatch-val (fn ~@fn-tail)))
...
(defn remove-method
 ...
 (.remove_method multifn dispatch-val))
...
(defn get-method
  ...
  {:added "1.0"
   :static true}
  [multifn dispatch-val] (.get_method multifn dispatch-val))
...
(defn eval
  ...
  {:added "1.0"
   :static true}
  [form] (.evaluate Hammock.Compiler form))
...
;         err (fn [& msg] (throw (ArgumentError. (apply str msg))))
...
(defonce ^:dynamic
  ^{:private true
     :doc "An atom of a set of symbols representing loaded libs"}
  *loaded-libs* (atom (hash-set)))
...
;   and no clause matches, an ArgumentError is thrown.
...
;                   (last clauses)
;                   `(throw (ArgumentError. (str "No matching clause: " ~ge))))]
;     (if (> 2 (count clauses))
;       `(let [~ge ~e] ~default)
;       (let [pairs (partition 2 clauses)
;             assoc-test (fn assoc-test [m test expr]
;                          (if (contains? m test)
;                            (throw (ArgumentError. (str "Duplicate case test constant: " test)))
;                            (assoc m test expr)))
;             pairs (reduce1
;                        (fn [m [test expr]]
;                          (if (seq? test)
...
;   [fdecl]
;   (when (empty? fdecl) (throw (ArgumentError.
...
;     (when bad-args
;       (throw (ArgumentError. (str "Parameter declaration \"" (first bad-args)
;                                              "\" should be a vector"))))))
